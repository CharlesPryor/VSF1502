...
NSString * Pryor_Charlie_Project1$ModuleRequireFormat = @"(function(exports){"
...
@implementation Pryor_Charlie_Project1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_pryor_charlie_project1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Pryor_Charlie_Project1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Pryor_Charlie_Project1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_pryor_charlie_project1 = [[Pryor_Charlie_Project1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_pryor_charlie_project1];
	
	NSString *_pryor_charlie_project1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _pryor_charlie_project1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_pryor_charlie_project1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Pryor_Charlie_Project1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.pryor_charlie_project1.kroll" 
