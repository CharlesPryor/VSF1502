...
NSString * Pryor_Charlie_project2$ModuleRequireFormat = @"(function(exports){"
...
@implementation Pryor_Charlie_project2Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_pryor_charlie_project2);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Pryor_Charlie_project2 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Pryor_Charlie_project2" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_pryor_charlie_project2 = [[Pryor_Charlie_project2Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_pryor_charlie_project2];
	
	NSString *_pryor_charlie_project2NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _pryor_charlie_project2NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_pryor_charlie_project2 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Pryor_Charlie_project2$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.pryor_charlie_project2.kroll" 
